...
function trackingHeaders(): Record<string, string> {
  ...
  const userId = process.env._USER_ID?.trim();
  const dashboardUrl = process.env.DASHBOARD_API_URL?.trim();

  if (userId) {
    headers["x-product-default-user-id"] = userId;
  }
  if (dashboardUrl) {
    headers["x-dashboard-api-url"] = dashboardUrl;
  }
